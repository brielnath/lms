...
mtrace("==================================================");
mtrace("🚀 FIXING QUIZ WITH MOODLE NATIVE QUIZ_ADD_QUIZ_QUESTION API");
mtrace("==================================================");
...
// 1. Clean old quiz records & slots
...
// 3. Create Course Module with section = 75 (Section 8 row ID)
...
foreach ($questions_data as $qindex => $qdata) {
    ...
    // Moodle 4.5 Question Bank Entry
    $qbe = new stdClass();
    $qbe->questioncategoryid = $cat->id;
    $qbe->idnumber = 'Q_NATIVE_' . ($qindex + 1);
    ...
    // Add Question using quiz_add_quiz_question native helper!
    quiz_add_quiz_question($q_id, $quiz, $qindex + 1, 1.0);

    mtrace("     ✅ Quiz Add Question Native: Question ID {$q_id} attached to Quiz ID {$quiz_id}");
}

rebuild_course_cache($course->id, true);

mtrace("==================================================");
mtrace("🎉 NATIVE QUIZ ADD QUESTION SUCCESS! VALID CMID: {$cm_id}");
mtrace("==================================================");
